...
class TTFontParser:
    ...
    def readTTCHeader(self):
        ...
        if self.ttcVersion not in self.ttcVersions:
            raise TTFError(
                '"%s" is not a %s file: can\'t read version 0x%8.8x' % (self.filename, self.fileKind, self.ttcVersion))
    ...
    def getSubfont(self, subfontIndex):
        ...
        try:
            ...
            raise TTFError('TTC file "%s": bad subfontIndex %s not in [0,%d]' % (self.filename, subfontIndex,
                                                                                 self.numSubfonts - 1))
        self.seek(pos)
        self.readHeader()
        self.readTableDirectory()
    ...
    def checksumTables(self):
        ...
        for t in self.tables:
            ...
            if xchecksum != checksum:
                raise TTFError(
                    'TTF file "%s": invalid checksum %s table: %s (expected %s)' % (self.filename, hex32(checksum),
                                                                                    t['tag'], hex32(xchecksum)))

    def checksumFile(self):
        # Check the checksums for the whole file
        checksum = calcChecksum(self._ttf_data)
        if 0xB1B0AFBA != checksum:
            raise TTFError('TTF file "%s": invalid checksum %s (expected 0xB1B0AFBA) len: %d &3: %d' % (self.filename,
                                                                                                        hex32(checksum),
                                                                                                        len(self._ttf_data),
                                                                                                        (
        ...
        for code in map(ord, text):
            ...
            if code in assignments:
                ...
                if 32 not in assignments:
                    # Ensure space is in assignments
                    if 32 in charToGlyph:
                        assignments[32] = 32
                        if 32 not in subsets[0]:
                            subsets[0].append(32)
                    n = 32
            else:
                if state.frozen:
                    raise pdfdoc.PDFError(
                        "Font %s is already frozen, cannot add new character U+%04X" % (self.fontName, code))
                n = state.nextCode
                if n & 0xFF == 32:
